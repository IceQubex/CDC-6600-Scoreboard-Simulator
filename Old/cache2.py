...
class scoreboard:
    ...
    def __init__(self, config_file):
        ...
        # set this value to 1 for more detailed output in the terminal
        self.verbose = 1
        ...
        if self.stall != 1:
            ...
            if inst.is_ALU_inst():
                ...
                if block == 0:
                    ...
                    if self.verbose == 1:
                        print(f"Instruction \"{inst.inst}\" is issued!")
    ...
    def read_inst(self, inst):
        ...
        if not inst.is_branch_inst():
            ...
            for i in range(1, len(inst.args)):
                # if inst.args[i] != -1 and inst.args[i] in self.tobewritten and inst.args[i] != inst.args[0]:
                #     block = 1
                if inst.args[i] != -1:
    ...
    def write_inst(self, inst):
        ...
        if inst.keyword == "daddi":
            if self.verbose == 1:
                print(
                    f"Register {inst.args[0]} = Register {inst.args[1]} + {int(inst.args[2])} = {self.R[inst.args[1]] + int(inst.args[2])}")
            self.R[inst.args[0]] = self.R[inst.args[1]] + int(inst.args[2])
        if inst.keyword == "dsub":
            if self.verbose == 1:
                print(
                    f"Register {inst.args[0]} = Register {inst.args[1]} - Register {inst.args[2]} = {self.R[inst.args[1]] - self.R[inst.args[2]]}")
    ...
    def I_cache_access(self, inst):
        ...
        if inst.pc // self.I_block_size in self.I_cache:
            ...
            if self.bus == 0:
                if inst.status == 0:
                    if self.verbose == 1:
                        print(f"Instruction \"{inst.inst}\" is a miss!")
                ...
                if inst.status == 0:
                    if self.D_cache[set][0][1] <= self.D_cache[set][1][1]:
                        if self.D_cache[set][0][2] == 1:
                            inst.status = 8*3
                            print(f"Instruction \"{inst.inst}\" needs to be put in a dirty block!")
                        else:
                            inst.status = 4*3
                    else:
    ...
    def simulate(self, instructions_list):
        ...
        while (self.end != 1):
            ...
            self.clock_cycle += 1
            print(f"At clock cycle {self.clock_cycle}: {self.bus}")
            ...
            while i < len(self.active):
                ...
                if not self.active[i].is_branch_inst() and not self.active[i].is_jump_inst() and not self.active[
                    i].is_hlt_inst():
            ...
            print("Active:", print(self.tobewritten))
            for i in range(len(self.active)):
                self.active[i].print()
