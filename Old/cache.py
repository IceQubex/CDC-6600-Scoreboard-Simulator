...
class scoreboard:
        ...
        # if the instruction is a branch instruction
            ...
            if block == 0:
                ...
                if (inst.keyword == "bne" and self.R[inst.args[0]] != self.R[inst.args[1]]) or (inst.keyword == "beq" and self.R[inst.args[0]] == self.R[inst.args[1]]):
                    self.pc = branch[inst.args[2]]-1

                    # flush the pipeline if branch is taken
                    print("BRANCH TAKEN!!!")
                    for k in range(len(self.active)):
                        if self.active[i] != inst:
                            self.active[i].status2 = 1
    ...
    def exec_inst(self, inst):
        ...
        # if the instruction is an L.W or S.W instruction --> 1 clock cycle:
    ...
    def write_inst(self, inst):
        ...
        if inst.keyword == "daddi":
            print(self.R)
            print(inst.args)
            print(self.R[inst.args[0]], self.R[inst.args[1]], inst.args[2])
    ...
    def simulate(self, instructions_list):
        ...
        return self.complete, len(self.complete), len(self.complete) - self.totalImiss, self.totalD, self.totalDmiss
