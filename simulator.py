...
class inst:
    def __init__(self, inst, pc):
        ...
        if ":" in inst:
            temp = inst[inst.index(":") + 1:].strip().split()
        else:
            temp = inst.split()
    ...
    def print(self):
        print(self.inst + " || ", end="")
        print(self.convert(self.fetch), self.convert(self.issue), self.convert(self.read), self.convert(self.exec), self.convert(self.write), self.raw, self.waw, self.struct)
    ...
    # function to check if the instruction is a jump instruction
    def is_jump_inst(self):
        if self.keyword == "j":
            return True
        return False

    # function to check if the instruction is a hlt instruction
...
class scoreboard:
    ...
    # method to prepare the scoreboard variables and status monitors
    def __init__(self, inst_file, data_file, config_file, result_file, verbosity=2, **kwargs):
        ...
        with open(inst_file, 'r') as f:
            self.instructions_list = f.read()
            self.instructions_list = self.instructions_list.split("\n")
            while "" in self.instructions_list:
                self.instructions_list.remove("")
        ...
        self.branch_table = {}
        for i, j in enumerate(self.instructions_list):
            if ":" in j:
                self.branch_table[j.split(":")[0].strip()] = i
        ...
        # set the file path where result is to be printed
        ...
        # define the list of active and completed instructions, as well as a list of registers which are going to be written
        self.active = []
        self.complete = []
        self.tobewritten = []
    ...
    def issue_inst(self, inst):
        ...
        if self.stall != 1:
            ...
            # if the instruction is a jump instruction
                ...
                # stall the pipeline
                self.stall = 1
                self.branch = self.branch_table[inst.args[0]]
                ...
                for k in range(len(self.active)):
                    self.active[k].flush = 1


            # if it is any other instruction (HLT)
            else:
                if self.verbose >= 1:
                    print(f"Instruction \"{inst.inst}\" completes issue stage!")
                inst.issue = self.clock_cycle
        ...
        # if the instruction is a branch instruction
            ...
            if block == 0:
                ...
                if (inst.keyword == "bne" and self.R[inst.args[0]] != self.R[inst.args[1]]) or (
                    ...
                    self.branch = self.branch_table[inst.args[2]]
    ...
    def free(self, inst):
        ...
        if not inst.is_branch_inst() and not inst.is_jump_inst() and not inst.is_hlt_inst():
    ...
    def D_cache_access(self, inst, data):
        ...
        # calculate the set it belongs to
        set = (data // 16) % 2
    ...
    # method to simulate the pipeline and write the result as output
    def simulate(self):
        ...
        while (self.end != 1):
            ...
            if self.oldpc != self.pc and self.stall != 1 and self.pc < len(self.instructions_list):
                self.active.append(inst(self.instructions_list[self.pc], self.pc))
                self.oldpc = self.pc
            ...
            while i < len(self.active):
                if self.active[i].fetch == 0:
                    ...
                    if self.active[i].is_branch_inst() and self.active[i].flush == 1:
                        break
            ...
            while i < len(self.active):
                ...
                elif self.active[i].flush != 0 and self.active[i].status == 0:
                    self.complete.append(self.active.pop(i))
                    continue
                i += 1
            ...
            # condition to check whether the pipeline simulation has ended
            if not self.pc < len(self.instructions_list):
                for i in range(len(self.active)):
        ...
        self.complete.sort(key=lambda x: x.fetch)

        with open(self.result_file, 'w') as f:
            f.write("Instruction\t\t\tFetch\tIssue\tRead\tExec\tWrite\tRAW\tWAW\tStruct\n")
            for i in range(len(self.complete)):
                if self.complete[i].fetch == 0:
                    continue
                if len(self.complete[i].inst) < 8:
                    f.write(self.complete[i].inst + "\t\t\t\t")
                elif len(self.complete[i].inst) < 16:
                    f.write(self.complete[i].inst + "\t\t\t")
                elif len(self.complete[i].inst) < 24:
                    f.write(self.complete[i].inst + "\t\t")
                else:
                    f.write(self.complete[i].inst + "\t")

                f.write(f"{inst.convert(self.complete[i].fetch)}\t{inst.convert(self.complete[i].issue)}\t{inst.convert(self.complete[i].read)}\t{inst.convert(self.complete[i].exec)}\t{inst.convert(self.complete[i].write)}\t{self.complete[i].raw}\t{self.complete[i].waw}\t{self.complete[i].struct}\n")

            f.write("\n\n")
            f.write(f"Total number of access requests for instruction cache: {len(self.complete)}\n")
            f.write(f"Number of instruction cache hits: {len(self.complete) - self.totalImiss}\n")
            f.write(f"Total number of access requests for data cache: {self.totalD}\n")
            f.write(f"Number of data cache hits: {self.totalD - self.totalDmiss}")

# create the scoreboard instance by passing in the command-line arguments
if len(sys.argv) == 6:
    s = scoreboard(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], verbosity=sys.argv[5])
elif len(sys.argv) == 5:
    s = scoreboard(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
else:
    print("Incorrect number of arguments passed!\n")
    print("Usage:\npython3 simulator.py <inst.txt> <data.txt> <config.txt> <result.txt>")
    exit()
